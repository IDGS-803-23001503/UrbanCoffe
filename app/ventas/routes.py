...
from model import DetalleVenta, ProductoTerminado, Venta, db
...
def tiendaCliente():
	...
	productos = ProductoTerminado.query.filter_by(activo=True).order_by(ProductoTerminado.nombre.asc()).all()
	return render_template("ventas/index.html", productos=productos)
...
def comprarProducto():
	...
	producto = ProductoTerminado.query.get(productoId)
